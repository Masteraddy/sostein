...
export const getOutRequestLocal = async (host, user) => {
  ...
  try {
    const res = await fetch(`${URL}/api/requests/out`);
...
export const deleteOutRequest = (id, user) => (dispatch) => {
  ...
  dispatch(inProgress());
  axios
    .delete(`${URL}/api/requests/out/${id}`, {
      headers: {
...
export const changeOutRequestStatus = (body, id, user) => (dispatch) => {
  ...
  axios
    .patch(`${URL}/api/requests/out/${id}`, data, {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `${token}`,
